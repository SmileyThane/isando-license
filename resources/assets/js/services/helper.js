...
export default {
    ...
    getInstanceStatus(instance){
        ...
        if(instance.status === 'pending')
            status.push({'color': 'info','label': i18n.instance.pending});
        else if(instance.status === 'suspended')
            status.push({'color': 'warning','label': i18n.instance.suspended});
        else if(instance.status === 'terminated')
            status.push({'color': 'danger','label': i18n.instance.terminated});

        if (instance.is_trial)
            status.push({'color': 'danger','label': i18n.instance.trial});

        if(instance.status === 'running')
            status.push({'color': 'success','label': i18n.instance.running});
        else if (instance.date_of_expiry < moment().format('YYYY-MM-DD'))
            status.push({'color': 'danger','label': i18n.instance.expired});
