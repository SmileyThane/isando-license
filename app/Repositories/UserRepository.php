...
class UserRepository
{
    ...
    public function paginate($params = array())
    {
        ...
        $query = $this->user->with('profile', 'roles')->filterByFirstName($first_name)->filterByLastName($last_name)->filterByEmail($email)->filterByRoleId($role_id)->filterByStatus($status)->createdAtDateBetween([
    ...
    public function deletable($id)
    {
        ...
        if ($user->hasRole(config('system.default_role.admin'))) {
            throw ValidationException::withMessages(['message' => trans('user.has_many_invoices')]);
        }
