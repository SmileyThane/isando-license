 ...
 *  @return string
 */
function currency($amount, $currency, $symbol = 0)
{
    $decimal_value = ($currency) ? $currency->decimal_place : 2;
    $position = ($currency) ? $currency->position : 'prefix';
...
/*
 *  Used to get path inside application
 *  @param
 *  $path as string
 *  @return string
 */
...
{
    return env('APP_PATH').$path;
}

/*
 *  Used to get path inside website
 *  @param
 *  $path as string
 *  @return string
 */
...
/*
 *  Used to generate Doughnut graph data
 */
function generateDoughnutGraphData($rows,$label){
        $graph = [
            'labels' => [],
            'data' => [],
            'color' => []
        ...
        foreach($rows as $key => $row){
            $graph['labels'][] = $row['name'];
            $graph['data'][] = $row['total'];
            $graph['color'][] = !empty($row['color']) ? $row['color'] : '#'.generateRandomColor();
        }
...
function hideEmail($email)
{
    ...
    $domain = end($em);
    $domain_len = floor(strlen($domain)/2);

    return substr($name,0, $len) . str_repeat('x', $len) . "@" . substr($domain,0, $domain_len) . str_repeat('x', $domain_len);   
}
